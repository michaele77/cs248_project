...
void Mesh::internalDraw(bool shadowPass, const Matrix4x4& worldToNDC) const {
    ...
    if (shadowPass) {
		...
		// 1. name sampler variable, texture array Id using getShadowTextureArrayId(), mimic the other texture samplers
		// set up object to light space
		// shader needs 2 inputs. an array of mat4 

		for (int j = 0; j < scene_->getNumSpotLights(); j++) {
			string varname = "shadowlight_mtx[" + std::to_string(j) + "]";
			shader_->setMatrixParameter(varname, scene_->getWorldToShadowLight(j)*objectToWorld); 
