...
void Scene::renderShadowPass(int shadowedLightIndex) {
    ...
    // world 3D to texture coordinate on shadow map--------------
    // take surface point, multiply with objtoworld.
    // worldspace to light space. create that matrix using task 1. eye, lookup and up
    // 1. World space to light space. Use task 1, fake camera at light source
    // multiply the two.

    auto fb_bind = gl_mgr_->bindFrameBuffer(shadowFrameBufferId_[shadowedLightIndex]); // for each light source store shadow map in a framebuffer
    //shadowFrameBufferId_[shadowedLightIndex] // array of ids of shadow maps

    Vector3D up = Vector3D(0,1,0);
    //Matrix4x4 worldToLight = createWorldToCameraMatrix(lightPos, camera_->getViewPoint(), up);
    Matrix4x4 worldToLight = createWorldToCameraMatrix(lightPos, lightPos + lightDir, up);
    Matrix4x4 projectionMat = createPerspectiveMatrix(fovy, aspect, near, far);

    Matrix4x4 worldToLightNDC = projectionMat * worldToLight;

    // 3. Shift range to 0-1. Use translation and scaling to get the right range. bias matrix
    //Matrix4x4::translation()
    //Matrix4x4::scal

    Matrix4x4 trans;

    trans(0, 0) = .5; trans(0, 1) = 0; trans(0, 2) = 0; trans(0, 3) = .5;
    trans(1, 0) = 0; trans(1, 1) = .5; trans(1, 2) = 0; trans(1, 3) = .5;
    trans(2, 0) = 0; trans(2, 1) = 0; trans(2, 2) = .5; trans(2, 3) = .5;
    ...
    //trans[2][0] = 0; trans[2][1] = 0; trans[2][2] = .5; trans[2][3] = .5;
    //trans[3][0] = 0; trans[3][1] = 0; trans[3][2] = 0; trans[3][3] =   1;
    worldToShadowLight_[shadowedLightIndex] = trans*worldToLightNDC;
